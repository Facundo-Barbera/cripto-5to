...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        node distance=1.4cm,
        every node/.style={draw, rounded corners, align=center, minimum width=3.2cm, font=\small},
        arrow/.style={-{Latex[length=3mm,width=2mm]}, thick}
    ]
        \node (root) {Zona Ra\'iz};
        \node (tld) [below=of root] {TLD .mx};
        \node (domain) [below=of tld] {Dominio firmado};
        \node (user) [below=of domain] {Usuario final};
        \node (service) [right=3.5cm of domain] {Servicios\\(Registros A/AAAA)};

        \draw[arrow] (root) -- node[right]{DS firmado} (tld);
        \draw[arrow] (tld) -- node[right]{Delegaci\'on segura} (domain);
        \draw[arrow] (domain) -- node[right]{DNSKEY + RRSIG} (user);
        \draw[arrow] (domain) -- node[above]{Traduce consultas} (service);
        \draw[arrow, dashed] (service) |- node[pos=0.25,right]{Respuestas autenticadas} (user);
    \end{tikzpicture}}
...
\textbf{Gobierno.} Aunque gob.mx y PEMEX exhiben una implementaci\'on ejemplar de DNSSEC, la mayor\'ia de las dependencias federales y estatales contin\'uan con la cadena de confianza abierta o inexistente. Esta situaci\'on expone expedientes sensibles, tr\'amites digitales y sistemas de recaudo, por lo que el rezago gubernamental debe convertirse en prioridad nacional.

\textbf{Educaci\'on.} La UNAM es el referente positivo y demuestra que la firma de la zona .mx es viable dentro del ecosistema acad\'emico. Sin embargo, instituciones como IPN, UDG y la mayor parte de las universidades privadas siguen sin publicar su DS o ni siquiera han iniciado el proceso, de modo que plataformas escolares, correos y servicios de pago permanecen vulnerables a manipulación.

\textbf{Sector financiero.} Ninguno de los bancos analizados cuenta con DNSSEC habilitado, lo que convierte a esta industria en la m\'as expuesta para la ciudadan\'ia. La ausencia de firmas digitales facilita ataques de redireccionamiento que pueden derivar en robo de credenciales, transferencias fraudulentas o interrupciones en los canales de banca en l\'inea.

\textbf{Sector comercial y medios.} Telecomunicaciones, supermercados y conglomerados medi\'aticos operan sin la protecci\'on criptogr\'afica, por lo que cada campa\~na de marketing, portal de compras o sitio de noticias se convierte en un vector de phishing altamente veros\'imil. Esta brecha impacta directamente al consumidor final, que interact\'ua con estos servicios de manera cotidiana sin verificaci\'on de identidad.
...
\begin{enumerate}
    ...
    \item NIC M\'exico. (s.f.). \textit{DNSSEC en .MX}. \url{https://www.nic.mx/es/InformacionGral/DNSSEC}
\end{enumerate}
