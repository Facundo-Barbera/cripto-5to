...
\label{anexo:docker}
Cuando se requiere levantar la aplicación completa y verificar la interfaz web local, es necesario contar con Docker Desktop instalado en el equipo (descarga disponible en \url{https://www.docker.com/products/docker-desktop/}). El proceso recomendado es el siguiente:
\begin{enumerate}
    \item Clonar el repositorio: \texttt{git clone https://github.com/Facundo-Barbera/DNSSEC-Analyzer.git}
    \item Abrir Docker Desktop y asegurarse de que el demonio esté en ejecución.
    \item Dentro de la carpeta del proyecto, ejecutar \texttt{docker compose up} para orquestar los servicios definidos.
    \item Una vez que los contenedores estén corriendo, acceder a \url{http://localhost:5050} para interactuar con la interfaz del analizador y revisar los resultados en tiempo real.
\end{enumerate}
Este flujo permite replicar la experiencia del socio formador y validar que los cambios realizados en el código se reflejen en la plataforma sin necesidad de configurar manualmente todas las dependencias.
